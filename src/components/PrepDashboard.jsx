...
const MOCK_SUBJECTS = [
  ...
  { id: "ppu", name: "Pengetahuan Umum", progress: 40, total: 25, completed: 10 },
  { id: "pbm", name: "Bacaan & Menulis", progress: 80, total: 20, completed: 16 },
  { id: "pk", name: "Pengetahuan Kuantitatif", progress: 55, total: 25, completed: 14 },
  { id: "lbi", name: "Literasi Bahasa Indonesia", progress: 70, total: 30, completed: 21 },
  { id: "lbe", name: "Literasi Bahasa Inggris", progress: 45, total: 25, completed: 11 },
  { id: "pm", name: "Penalaran Matematika", progress: 30, total: 25, completed: 8 },
];

const MOCK_RECENT_SESSIONS = [
  { id: 1, subject: "Penalaran Umum", score: 85, date: "2026-01-25", questions: 20 },
  { id: 2, subject: "Literasi Bahasa Inggris", score: 72, date: "2026-01-24", questions: 15 },
  { id: 3, subject: "Penalaran Matematika", score: 90, date: "2026-01-23", questions: 20 },
];
...
const StatCard = ({ icon, label, value, subValue, accent = false }) => (
  <div className={`p-4 rounded-lg ${accent ? "bg-blue-50 border border-blue-100" : "bg-stone-50"}`}>
    <div className={`flex items-center gap-2 mb-1 ${accent ? "text-blue-600" : "text-stone-500"}`}>
      {icon}
      <span className="text-sm font-medium">{label}</span>
    </div>
    <p className={`text-2xl font-bold ${accent ? "text-blue-700" : "text-stone-700"}`}>{value}</p>
  ...
  <div className="flex items-center justify-between p-3 bg-stone-50 rounded-lg hover:bg-blue-50 transition-colors group">
    <div className="flex-1 min-w-0">
      <div className="flex items-center justify-between mb-1">
        <h4 className="text-sm font-medium text-stone-700 truncate pr-2">{subject.name}</h4>
        <span className="text-xs text-stone-500 shrink-0">{subject.completed}/{subject.total}</span>
      </div>
...
const RecentSession = ({ session }) => (
  <div className="flex items-center justify-between py-3 border-b border-stone-100 last:border-0">
    ...
    <div className={`text-lg font-bold ${session.score >= 80 ? "text-green-600" : session.score >= 60 ? "text-yellow-600" : "text-red-500"}`}>
      {session.score}%
    </div>
  </div>
);
...
export default function PrepDashboard() {
  ...
  const accuracy = Math.round((MOCK_PROGRESS.answeredCorrectly / MOCK_PROGRESS.totalQuestions) * 100);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-500 rounded-xl p-6 text-white">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <h2 className="text-lg sm:text-xl font-bold mb-1">Lanjutkan Latihan</h2>
            <p className="text-blue-100 text-sm">
              Kamu sedang dalam streak {MOCK_PROGRESS.currentStreak} hari! Jangan putus.
            </p>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-stone-700">Progress Materi</h3>
        ...
        <div className="bg-white rounded-xl shadow-sm p-5">
          <h3 className="text-lg font-bold text-stone-700 mb-4">Aktivitas Terbaru</h3>
      ...
      <div className="bg-stone-50 rounded-xl p-5 border border-stone-200">
        ...
        <p className="text-stone-600 text-sm leading-relaxed">
          Fokus pada materi dengan progress terendah terlebih dahulu. Berdasarkan data latihanmu, 
          <span className="font-semibold text-blue-600"> Penalaran Matematika </span> 
          membutuhkan perhatian lebih. Luangkan 20-30 menit setiap hari untuk materi ini.
        </p>
      </div>
    </div>
  );
